...
that end before the dispatch boundary use an explicit zero-submission fact
instead of an invented attempt. When a physical attempt is known but terminal
provider fallback metadata is unavailable, scoped coverage lowers only the
derived provider-fallback total and serving-model identity; attempt and event
counts remain exact.
