      ...
      const result = await runCodeModeModelMatrix(
        { ...options(repoRoot), allowFailures: false },
        {
          ...
          runCell: async (params) => {
            ...
            rawNonces.push(params.frontierEvidenceRunNonce);
            rawCacheKeys.push(
              deriveTestPromptCacheKey(policy.contentDigestKey, params.frontierEvidenceRunNonce),
            );
