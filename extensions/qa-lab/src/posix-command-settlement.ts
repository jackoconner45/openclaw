...
type QaPosixCommandSettlementParams = {
  ...
  onSettled: (outcome: { cleanupFailure?: Error; primary: QaPosixCommandPrimary }) => void;
...
export function createQaPosixCommandSettlement(params: QaPosixCommandSettlementParams) {
  ...
  const settle = () => {
    ...
    const cleanupFailure =
      errors.length > 1 ? new AggregateError(errors, params.cleanupFailureMessage) : errors[0];
    dispose();
    params.onSettled({ primary, ...(cleanupFailure ? { cleanupFailure } : {}) });
    if (parentSignal) {
  ...
  const verify = () => {
    if (alive() && !windows) {
      errors.push(
        new Error(`${params.cleanupFailureMessage}: pgid=${params.processGroupId} alive`),
      );
    }
    if (!stdioDrained) {
      drainTimedOut = true;
  ...
  function onStdoutError(error: Error) {
    freeze({ type: "stream-error", stream: "stdout", error });
  }
  function onStderrError(error: Error) {
    freeze({ type: "stream-error", stream: "stderr", error });
  }
