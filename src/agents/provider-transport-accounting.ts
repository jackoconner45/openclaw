...
function retainEventIdentity(
  ...
  const existing = state.eventFingerprints.get(event.eventId);
  ...
  state.eventFingerprints.set(event.eventId, { fingerprint, type: event.type });
  return "accepted";
}
...
function correlateTransportEvent(
  event: AiModelTransportEvent,
  state: MutableProviderTransportAccounting,
): { callId?: string; call?: TrackedLogicalCall } | undefined {
  ...
  const call = state.logicalCalls.get(normalized.value);
  if (!call) {
    rejectFact(state, "transport_uncorrelated_event", "call_event");
    return undefined;
  }
  return { callId: normalized.value, call };
}
...
function requireOpenCall(
  ...
  const call = state.logicalCalls.get(event.callId);
  if (!call) {
...
export function createProviderTransportAccountingCollector(): ProviderTransportAccountingCollector {
  ...
  const observer: ProviderTransportAccountingObserver = {
    onLogicalCallStarted(rawCall) {
      ...
      const existing = state.logicalCalls.get(callId);
      if (existing) {
        if (!sameRequestedRoute(requestedRoute(existing), route)) {
    ...
    onLogicalCallSettled(rawCallId, rawOutcome, cachedInput = { state: "unknown" }) {
      ...
      const call = state.logicalCalls.get(callId);
      if (!call) {
    ...
    onTransportEvent(rawEvent) {
      ...
      try {
        ...
        if (
          !event ||
          retainEventIdentity(event, state) !== "accepted" ||
          !applyTransportEvent(event, state)
        ) {
          return;
        }
        if (correlation.call) {
