...
export const AGENT_EXEC_TRACE_SCHEMA_VERSION = 4 as const;
...
export type { AgentExecTraceCacheObservation, AgentExecTraceMetric };

type SanitizedFrontierReceipt = {
  receiptCount: number;
  valid: boolean;
  logicalCalls: number;
...
export type AgentExecTrace = {
  ...
  frontierEvidence?: SanitizedFrontierReceipt;
  metrics: {
...
): SanitizedFrontierReceipt | undefined {
  ...
  if (receipts?.length !== 1 || !receipts[0]) {
    ...
    if (
      ...
      receipt.callSequences.reduce(
        (count, sequence) =>
          count +
          sequence.requests.filter(
            (request) => request.payloadVariant === "encrypted-content-retry",
          ).length,
        0,
      ) !== transport.attempts.payloadRecoveries
    ) {
    ...
    if (transport) {
      const receiptBindingIds = receipt.callSequences.map(
      ...
      if (logicalCallIds.size !== transport.logicalCalls.entries.length) {
        reasons.add("cross_layer_call_order_mismatch");
      }
      for (const [callIndex, call] of transport.logicalCalls.entries.entries()) {
        const sequence = receipt.callSequences[callIndex];
        const attempts = transport.events.entries.filter(
          (event) => event.type === "attempt" && event.callId === call.callId,
        );
