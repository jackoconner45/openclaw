...
export function normalizedReasons(reasons: Iterable<string>): string[] {
...
export function observedMetric(params: {
  ...
  const reasons = coverageReasons(params.coverage);
  if (params.lowerBound === true || params.coverage.state !== "complete") {
    return {
      state: "lower_bound",
      value: params.value,
